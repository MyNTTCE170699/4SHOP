                            ...
                            <div class="card-body p-4 p-md-5">
                                <form action="/customer" method="post" id="updateForm">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input required name="fullname" type="text"  class="form-control" value="${sessionScope.roleToUpdate.getName()}"/>
                                    </div>

                                    <!-- Password input -->
                                    <div class="mb-4">
                                        <label class="form-label" for="form3Example4">User Name</label>
                                        <input required name="username" type="text" class="form-control" value="${sessionScope.roleToUpdate.getUserName()}" />
                                        <span style="color: red">${requestScope.usernameErr}</span>
                                    </div>                                  

                                    <div class="mb-4">
                                        <label class="form-label" for="form3Example4">Phone</label>
                                        <input required name="phone" type="tel" pattern="[0-9]{10}" class="form-control" value="${sessionScope.roleToUpdate.getPhone()}" />
                                        <span style="color: red">${requestScope.phoneErr}</span>
                                    </div>

                                    <div class="mb-4">
                                        <label class="form-label" for="form3Example3">Email</label>
                                        <input required name="email" type="email" class="form-control" value="${sessionScope.roleToUpdate.getEmail()}" />
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div style="display: flex; justify-content: space-evenly">
                                            <div>
                                                Male <input checked="true" name="gender" type="radio" value="Male" <c:if test="${sessionScope.roleToUpdate.getGender()=='Male'}">checked</c:if>/>
                                                </div>
                                                <div>
                                                    Female <input name="gender" type="radio" value="Female" <c:if test="${sessionScope.roleToUpdate.getGender()=='Female'}">checked</c:if>/>
                                                </div> 
                                            </div>
                                        </div>
